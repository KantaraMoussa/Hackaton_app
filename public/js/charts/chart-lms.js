...
!function (NioApp, $) {
  ...
  //-------------------------- code a optisé par JSON
  var TitleDepartementChartList=document.querySelectorAll(".list-department-dashboard");
  var departmentList=[];
  for (let i = 0; i < TitleDepartementChartList.length; i++) {
    const element = TitleDepartementChartList[i].lastElementChild.firstChild.textContent;
    departmentList.push(element);
  }
 //-------------------------- FIN code a optisé par JSON
  ...
  var TrafficChannelDoughnutData = {
    labels: ["Boursiers", "Non-boursiers", "Exempté", "Others"],
    dataUnit: 'People',
    legend: false,
    datasets: [{
      borderColor: "#fff",
...
var trafficSources = {
  labels: ["Etudiant Boursiers", "Formation Payante", "Formation Continue", "Autres"],
  dataUnit: 'People',
  legend: false,
  datasets: [{
    borderColor: "#fff",
    background: ["#b695ff", "#b8acff", "#79f1dc", "#f9db7b"],
    data: statistic
  }]
};

function lmsDoughnutS1x(selector, set_data) {
